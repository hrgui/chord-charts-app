...
const pouchDbAdapter = {
  [ApiMethod.get]: (args: PouchDbBaseQueryArgs) =>
    getDb().get(args.id as string),
  ...
  [ApiMethod.update]: (args: Required<PouchDbBaseQueryArgs>) =>
    getDb().put(args.body),
  [ApiMethod.delete]: (args: Required<PouchDbBaseQueryArgs>) =>
    getDb().remove(
      args.body as { [name: string]: any; _id: string; _rev: string }
    ),
};

const pouchDbBaseQuery = async (args: PouchDbBaseQueryArgs) => {
  try {
    console.log("pouchDbBaseQuery args", args);
    const res = await pouchDbAdapter[args.method](
      args as Required<PouchDbBaseQueryArgs>
    );
